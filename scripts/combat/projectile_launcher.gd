...
## Fires one projectile each time it is used: what the Slingshot does
## (scenes/actions/slingshot.tscn). It creates a `projectile_scene` at its user's position and
## launches it the way the user faces. The projectile then flies and hits things by itself
## (see Projectile), so neither the launcher nor Carl knows anything about its flight.
## Nothing is used up: the only limit is the cooldown.
## A future ranged weapon is another scene with this node and its own numbers and projectile.
