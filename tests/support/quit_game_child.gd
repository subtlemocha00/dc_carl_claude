...
## Helper for test_quit_game.gd, which starts it as a separate Godot process. Never run it on
## its own: it plays a real session that ends by closing the game, and prints what it saw as
## "CHILD: ..." lines for the parent test to check.
...
func _use_test_save_file() -> void:
	...
	if not save_path.simplify_path().begins_with(TEST_SAVE_FOLDER) or _mode not in ["pause_quit", "window_close"]:
